...
filename <- "Z:/Group_work/Alex/Encrypted/Alex/PR/PR clinical 2019 Jun-Nov/Logs/PR_clinical_2019_Jun-Nov_cleaning_log_"
filedate <- Sys.Date()
...
# To see if we can just run our code straight away, we read it in:
datprev <- read.csv("Z:/Group_work/Alex/Encrypted/Alex/PR/PR clinical 2019 interim/Data/rawData/PR-extract-v401-201903-201908/PR-extract-v401.csv", header = TRUE, 
                           stringsAsFactors = TRUE, na.strings = c("NA", ""))


datprev <- datprev %>% rename(study_ID = StudyID,
...
datprev <- datprev %>% mutate(study_ID = as.character(study_ID),
...
dat <- read.csv("Z:/Group_work/Alex/Encrypted/Alex/PR/PR clinical 2019 Jun-Nov/Data/rawData/NACAP-PR-20190601-2019-11-30-v100-IMPERIAL.csv", header = TRUE, 
                stringsAsFactors = TRUE, na.strings = c("NA", ""))

colnames(dat)
# Need to add in the country variable myself...
# Doing it this way so that it fits in with how Tim would usually give it to me, so I can just take
# it out next time if need be.

dat <- dat %>% mutate(Country = NA,
                      firstletter = substr(dat$Trust.Code, 1, 1))

dat$Country[dat$firstletter == 7] <- "WA"
dat$Country[dat$firstletter == "S"] <- "SC"
dat$Country[dat$firstletter != "S" & dat$firstletter != 7] <- "EN"
dat$Country <- factor(dat$Country)

dat <- dat %>% select(-firstletter)
...
dat <- dat %>% rename(study_ID = StudyID,
                      ...
                      enrolled = X5.1.Enrolled.onto.a.PR.programme,
                      start_date = X5.1a.Start.Date,
                      centre_based = X5.2.Centre.based.PR.programme,
                      prog_type = X5.2a.Programme.Type,
                      scheduled_sess_centre_no = X5.2b.Centre.based.PR.Sessions.Scheduled,
                      rec_sess_centre_group_no = X5.2c.a..Group.centre.based.PR.sessions.received,
                      rec_sess_centre_indiv_no = X5.2c.b..1.1.centre.based.PR.sessions.received,
                      home_based = X5.3.Home.based.PR.programme,
                      scheduled_sess_home_no = X5.3a.Home.based.PR.Sessions.Scheduled,
                      rec_sess_home_in_person_no = X5.3b.a..Home.based.PR.Sessions...In.Person,
                      rec_sess_home_video_group_no = X5.3b.b..Home.based.PR.Sessions...Video.conferencing...group,
...
dat <- dat %>% select(study_ID:patient_ID, country, trust_code:dataset_version)
...
# Now we compare the two datasets.

# This hashed out bit below was just to check that the code I wrote below was doing what I thought
# it was.
# dat$country <- as.character(dat$country)
# dat$country[1:10] <- "MYsterY!!"
# dat$country <- factor(dat$country)

for (i in colnames(dat)) {
  comp1 <- pull(dat, i)
  comp2 <- pull(datprev, i)
  print(i)
  print(attr.all.equal(comp1, comp2))
}
...
dat <- dat %>% mutate(org_code = as.character(org_code),
                      trust_code = as.character(trust_code))



# We need to attach the names as well

PRnames <- read.csv(
  "Z:/Group_work/PS_AA/PR/PR clinical 2019 Jun-Nov/PR trust names/NACAP-PR-ServiceCodes-2004.csv",
  stringsAsFactors = FALSE)


PRnames <- PRnames %>% rename(org_code = Code, org_name = Name, trust_code = TrustCode,
                              trust_name = Trust)


# Issues are:

# NTV6 + NTV trust_code + org_code
# RDY1 + NR5
# RNL2 + RNL
...
# RNL2 + RNL/RNN -> RNL2 + RNN (org code + trust code): -> Trust no longer exists as there's been a merger, will update to new trust name. I think this is the best way
# to do it as, at a service level, it doesn't help for them to receive two reports, one for the same service at one trust and
# one for the same service at a different trust.


dat %>% filter(trust_code %in% c("RNL2", "RNN", "RNL")) %>% select(trust_code, org_code) %>% table()

dat$trust_code[dat$org_code == "RNL2" & dat$trust_code == "RNL"] <- "RNN"



# For NR5 and RDY1:

# In brief: in the latest PR extract 22 records with both unit/org marked 'RDY1' AND trust code 'NR5' should be re-assigned
# the RDY trust code instead of NR5. These records are outlined in the spreadsheet attached, (with study ID pseudo references).
# This issue arose in September 2019.
...
dat %>% select(trust_code) %>% filter(trust_code == "RDY") %>% table()

dat$trust_code[dat$org_code == "RDY1" & dat$trust_code == "NR5"] <- "RDY"



# For NTV6 + NTV:

# We understand that the service 'XXX' (code NTV6) and assigned 
# to the trust 'XXX' (trust code NTV) was changed to a new service name: 
# 'XXX' (retaining the same code NTV6) and assigned to a new trust 
# 'XXX' (RVR)
# in August 2019.
...
dat$trust_code[dat$org_code == "NTV6" & dat$trust_code == "NTV"] <- "RVR"


# And now... we join them together.


dat <- left_join(dat, PRnames[c("org_code", "trust_code", "org_name", "trust_name")],
                 by = c("org_code", "trust_code"))


# Now we see what we're left with

dat %>% filter(is.na(trust_name) | is.na(org_code)) %>% select(org_code, trust_code, org_name, trust_name) %>% arrange(org_code)

dat %>% filter(trust_code == "RVR") %>% select(org_code, trust_code, trust_name, org_name)




# # Code updates 22 people as expected
# 
# 
# 
# 
# dat %>% group_by(trust_code, org_code) %>% summarise(n()) %>% print(n=1000)
# 
# # dat <- dat %>% mutate_at(vars(org_code, trust_code), .funs = list(~factor(.)))
# 
...
# dat %>% group_by(trust_code, org_code) %>% summarise(n()) %>% print(n=1000)
# 
# # Three combos that don't fit with the updated PR list document:
# 
# # NTV6 + NTV: -> can just use NTV
# # RDY1 + NR5: -> Tim needs to check what to do with this
# # RNL2 + RNL: -> Trust no longer exists, will mark this in the trust name
# 
# 
# # So, we do these ones individually:
# 
# PRnames %>% filter(org_code == "NTV6" | org_code == "RDY1" | org_code == "RNL2")
# 
# PRnames %>% filter(trust_code == "NTV" | org_code == "NR5" | org_code == "RNL")
# 
# dat %>% filter(org_code == "NTV6" | org_code == "RDY1" | org_code == "RNL2") %>% select(org_code, trust_code) %>% table()
# 
# dat$org_name[dat$org_code == "NTV6"] <- PRnames %>% filter(org_code == "NTV6") %>% select(org_name) %>%
#                                            pull()
# dat$trust_name[dat$trust_code == "NTV" & dat$org_code == "NTV6"] <- PRnames %>% 
#                                                 filter(trust_code == "NTV") %>% select(trust_name) %>% pull()
# 
# 
# dat$org_name[dat$org_code == "RDY1"] <- PRnames %>% filter(org_code == "RDY1") %>% select(org_name) %>%
#   pull()
# dat$trust_name[dat$trust_code == "NR5" & dat$org_code == "RDY1"] <- "Await confirmation from Tim"
# 
# 
# dat$org_name[dat$org_code == "RNL2"] <- PRnames %>% filter(org_code == "RNL2") %>% select(org_name) %>%
#   pull()
# dat$trust_name[dat$trust_code == "RNL" & dat$org_code == "RNL2"] <- "Trust no longer exists due to merger"
# 
# 
# # dat %>% filter(org_code == "NTV6" | org_code == "RDY1" | org_code == "RNL2") %>% 
# #   select(org_code, trust_code, org_name, trust_name) %>% arrange(org_code)
# 
# dat %>% filter(org_code == "RNL2") %>% 
#   select(org_code, trust_code, org_name, trust_name) %>% arrange(org_code)


dat$org_code <- factor(dat$org_code)
dat$trust_code <- factor(dat$trust_code)
dat$org_name <- factor(dat$org_name)
dat$trust_name <- factor(dat$trust_name)



summary(dat$org_code)
summary(dat$trust_code)
summary(dat$org_name)
summary(dat$trust_name)



# Need to deal with the 7 records for which the trust was subject to a merger.
# Doing this by coding the trust name as 'trust no longer exists' but keeping everything else the same.

# dat %>% filter(is.na(trust_name)) %>% select(org_code, trust_code, org_name, trust_name) %>% arrange(org_code)
# 
# dat %>% filter(org_code == "RNL2") %>% select(org_code, trust_code, org_name, trust_name)


# Okay for now - just need to worry about it again when I do the benchmarking section and org-level analysis.
...
# an 'N/A' option for cvd, musculoskeletal, and mental illness, N = 14-19 for each option
dat %>% select(CVD_history_orig, musc_skel_history_orig, mental_history_orig) %>% summary()
...
# Originally, we created new health history variables that group 'No' and 'N/A' together, as the 
# 'N/A' was added halfway through the analysis.

# This time, we want to simply recode 'N/A' as 'NA' (missing)

# The hashed out bit below is what we used to do:
...
dat <- dat %>% mutate(CVD_history = as.character(CVD_history_orig),
                      musc_skel_history = as.character(musc_skel_history_orig),
                      mental_history = as.character(mental_history_orig))

dat$CVD_history[dat$CVD_history == "N/A"] <- NA
dat$musc_skel_history[dat$musc_skel_history == "N/A"] <- NA
dat$mental_history[dat$mental_history == "N/A"] <- NA

dat$CVD_history <- factor(dat$CVD_history)
dat$musc_skel_history <- factor(dat$musc_skel_history)
dat$mental_history <- factor(dat$mental_history)

dat <- dat %>% mutate(CVD_history_var_for_NA_N_only = fct_recode(CVD_history_orig, `Not N/A` = "No",
                                                                 `Not N/A` = "Yes"),
                      musc_skel_history_var_for_NA_N_only = fct_recode(musc_skel_history_orig, 
                                                                       `Not N/A` = "No", `Not N/A` = "Yes"),
                      mental_history_var_for_NA_N_only = fct_recode(mental_history_orig, 
...
summary(dat$CVD_history)
summary(dat$CVD_history_orig)
...
summary(dat$anxiety_bin)
summary(dat$depression_bin)
summary(dat$SMI_bin)
...
dat %>% select(test_type_init, ESWT_at_init) %>% table()
...
dat$all_3_test_types_dis <- factor(dat$all_3_test_types_dis)
dat$who_is_ESWT_given_to_dis <- factor(dat$who_is_ESWT_given_to_dis)

summary(dat$who_is_ESWT_given_to_dis)
...
summary(dat$rec_sess_centre_no)
summary(dat$rec_sess_home_no)
...
summary(dat$age)
dat <- dat %>% mutate(agecat = cut(age, breaks = c(35, 45, 55, 65, 75, 85, Inf), right = FALSE,
...
# Make the new variables we need for the new analysis we've been asked to do

summary(dat$discharge_assess_bin)
summary(dat$MRC_score_init)
...
nlc("How many people are in the dataset who were assessed before 1st June 2019?")

dat %>% filter(assess_date < "2019-06-01") %>% nrow()

nlc("How many people are assessed after the 30th November?")

dat %>% filter(assess_date > "2019-11-30") %>% nrow()


nlc("We filter these out and are left with this many in the dataset who were assessed between
1st March and 31st May:")

dat <- dat %>% filter(assess_date >= "2019-06-01") %>% filter(assess_date <= "2019-11-30") 
...
reporttabs <- paste0("Z:/Group_work/PS_AA/PR/PR clinical 2019 Jun-Nov/Data/tidyData/PR_SCC_Jun-Nov_report_tables_",
                     Sys.Date(), ".csv")
...
WTfreq(dat, "anyIMD") 
# WTfreq(dat, "ethnicity")  removed
WTfreq(dat, "ref_location") 
WTmed(dat, "ref_to_start_days_stable_COPD")  # Not normal


WTfreq(dat, "ninety_day_referral_to_start_for_stable_COPD") 
WTfreq(dat, "thirty_day_referral_to_start_for_AECOPD") 
# WTmed(dat, "assess_to_start_days")  # Not normal
WTmed(dat, "assess_to_start_days_stable_COPD")
...
WTfreq(dat, "FEV1_percpred_rec") 
WTmed(dat, "FEV1_percpred")  # Not normal
WTfreq(dat, "FEV1FVC_rec") 

# This needs to be done with 2 decimal places
WTmed(dat, "FEV1FVC", 2) #  # Not normal


WTfreq(dat, "BMI_rec") 

# This needs to be done with 1 decimal place
WTmed(dat, "BMI", 1)  # Not normal


WTfreq(dat, "MRC_score_init") 
WTfreq(dat, "CVD_history") 
WTfreq(dat, "CVD_history_var_for_NA_N_only") 
WTfreq(dat, "musc_skel_history") 
WTfreq(dat, "musc_skel_history_var_for_NA_N_only") 
WTfreq(dat, "mental_history") 
WTfreq(dat, "mental_history_var_for_NA_N_only") 
WTfreq(dat, "anxiety_bin") 
WTfreq(dat, "depression_bin") 
WTfreq(dat, "SMI_bin") 
...
WTfreq(dat, "all_3_test_types_init")  # Needs extra work

# FreqSum(dat, "who_is_ESWT_given_to_init") 


WTmed(dat, "test_value_ISWT_init")  # Not normal
WTfreq(dat, "prac_test_ISWT_init") 
WTmed(dat, "test_value_6MWT_init")  # Not normal
WTfreq(dat, "prac_test_6MWT_init") 
WTmed(dat, "ESWT_value_init")  # Not normal
# WTfreq(dat, "who_only_gets_ESWT_init") - This is just confusing if I keep it in
WTfreq(dat, "which_other_test_with_ESWT_init") 

WTfreq(dat, "CAT_init") 
WTfreq(dat, "CRQ_init") 
WTmed(dat, "CAT_score_init", 0)  # To 0 decimal places, Reasonably normal
WTmed(dat, "CRQ_dyspnoea_init", 1)  # Variable is to 1 DP
WTmed(dat, "CRQ_fatigue_init", 1)  # Variable is to 2 DP

# head(dat$CAT_score_init, 30)
# head(dat$CRQ_fatigue_init, 30)
# head(dat$CRQ_dyspnoea_init, 30)
# head(dat$CRQ_emotion_init, 30)
# head(dat$CRQ_mastery_init, 30)



WTmed(dat, "CRQ_emotion_init", 1)  # Variable is to 2 DP
WTmed(dat, "CRQ_mastery_init", 1)  # Variable is to 2 DP
WTfreq(dat, "enrolled")    # seems like a lot of issues are getting patients to turn up 
# to assessment maybe?
WTfreq(dat, "PR_location") 
WTfreq(dat, "prog_type") 
WTmed(dat, "scheduled_sess_centre_no") 
WTfreq(dat, "rec_sess_centre_group_bin") 
WTfreq(dat, "rec_sess_centre_indiv_bin") 
WTmed(dat, "rec_sess_centre_group_no") 
WTmed(dat, "rec_sess_centre_indiv_no") 
WTmed(dat, "rec_sess_centre_no") 



WTmed(dat, "scheduled_sess_home_no") 
WTfreq(dat, "rec_sess_home_in_person_bin") 
WTfreq(dat, "rec_sess_home_video_group_bin") 
WTfreq(dat, "rec_sess_home_video_indiv_bin") 
WTfreq(dat, "rec_sess_home_phone_bin") 
WTfreq(dat, "rec_sess_home_other_bin") 

WTmed(dat, "rec_sess_home_in_person_no") 
WTmed(dat, "rec_sess_home_video_group_no") 
WTmed(dat, "rec_sess_home_video_indiv_no") 
WTmed(dat, "rec_sess_home_phone_no") 
WTmed(dat, "rec_sess_home_other_no") 
WTmed(dat, "rec_sess_home_no") 
WTfreq(dat, "discharge_assess_bin") 

completion_ratio <- data.frame(completion_ratio_eng = sprintf("%.2f", 
                                     round(nrow(filter(dat, country == "England" & discharge_assess_bin == "Yes"))/
                                     nrow(filter(dat, country == "England" & discharge_assess_bin == "No")), 1)),
                               completion_ratio_scot = sprintf("%.2f", 
                                    round(nrow(filter(dat, country == "Scotland" & discharge_assess_bin == "Yes"))/
                                         nrow(filter(dat, country == "Scotland" & discharge_assess_bin == "No")), 1)),
                               completion_ratio_wales = sprintf("%.2f", 
                                    round(nrow(filter(dat, country == "Wales" & discharge_assess_bin == "Yes"))/
                                         nrow(filter(dat, country == "Wales" & discharge_assess_bin == "No")), 1)),
                               completion_ratio_all = sprintf("%.2f", round(nrow(filter(dat, discharge_assess_bin == "Yes"))/
                                                                      nrow(filter(dat, discharge_assess_bin == "No")), 1)))

completion_ratio

# Need for each country

write.table(completion_ratio, 
            file = reporttabs, sep = "\t", append = TRUE, 
            quote = FALSE,
            col.names = TRUE, row.names = FALSE)
...
WTfreq(dat, "discharge_assess_no_reason") 
WTfreq(dat, "PR_type_location") 
WTfreq(dat, "discharge_assess_bin_by_rolling") 
WTfreq(dat, "discharge_assess_bin_by_cohort") 
WTfreq(dat, "discharge_assess_bin_by_home") 
WTfreq(dat, "discharge_assess_bin_by_both") 

WTfreq(dat, "discharge_assess_bin_by_MRC1_init")
WTfreq(dat, "discharge_assess_bin_by_MRC2_init")
WTfreq(dat, "discharge_assess_bin_by_MRC3_init")
WTfreq(dat, "discharge_assess_bin_by_MRC4_init")
WTfreq(dat, "discharge_assess_bin_by_MRC5_init")
WTfreq(dat, "discharge_assess_bin_by_MRC_NR_init")
...
WTfreq(dat, "exercise_plan_by_rolling") 
WTfreq(dat, "exercise_plan_by_cohort") 
WTfreq(dat, "exercise_plan_by_home") 
WTfreq(dat, "exercise_plan_by_both") 
WTfreq(dat, "exercise_plan") 
WTmed(dat, "assess_to_discharge_days") 
WTfreq(dat, "MRC_score_dis") 
WTfreq(dat, "MRC_change_factor") 


grade.change.table <- dat %>% select(MRC_score_init, MRC_score_dis) %>% table() %>% as.data.frame.matrix() %>% rownames_to_column() %>% 
  as.matrix() %>% rbind(colnames(.), colnames(.), .)

grade.change.table[1, ] <- c("", "MRC_grade_dis", "", "", "", "", "")
grade.change.table[2, 1] <- "MRC_grade_init"
grade.change.table
  
write.table(grade.change.table, file = reporttabs, sep = "\t", append = TRUE, 
              quote = FALSE,
              col.names = FALSE, row.names = FALSE)

# Check that it's the same as this:
...
WTfreq(dat, "all_3_test_types_dis") 
WTfreq(dat, "who_is_ESWT_given_to_dis") 
WTmed(dat, "test_value_ISWT_diff")  # Not normal
WTmed(dat, "test_value_6MWT_diff")  # Not normal
WTmed(dat, "test_value_ESWT_diff")  # Not normal


WTfreq(dat, "MCID_N_ISWT_6MWT") 
WTfreq(dat, "MCID_ISWT") 
WTfreq(dat, "MCID_6MWT")  
WTfreq(dat, "CAT_CRQ_dis_N") 
WTfreq(dat, "CAT_dis") 
WTfreq(dat, "CRQ_dis") 
WTfreq(dat, "CAT_CRQ_score_diff_N") 
WTmed(dat, "CAT_score_diff", 0)  # Variable is to 0 DP
WTmed(dat, "CRQ_dyspnoea_diff", 1) # Variable is to 1 DP
WTmed(dat, "CRQ_fatigue_diff", 1) # Variable is to 2 DP
WTmed(dat, "CRQ_emotion_diff", 1) # Variable is to 2 DP
WTmed(dat, "CRQ_mastery_diff", 1) # Variable is to 2 DP
WTfreq(dat, "MCID_CAT") 
WTfreq(dat, "MCID_CRQ_dyspnoea")  
WTfreq(dat, "MCID_CRQ_fatigue")  
WTfreq(dat, "MCID_CRQ_emotion")  
WTfreq(dat, "MCID_CRQ_mastery") 
WTfreq(dat, "MCID_N_CAT_CRQ") 
WTfreq(dat, "MCID_N_CAT_CRQ") 
WTfreq(dat, "BM_start_90") 
WTfreq(dat, "BM_prac_test") 
WTfreq(dat, "BM_discharge_assess") 
WTfreq(dat, "BM_exercise_plan") 
WTfreq(dat, "BM_MCID_exercise") 
WTfreq(dat, "BM_MCID_CAT_CRQ") 
...
flat.org <- flat.org[ ,c(1, 1, 2:ncol(flat.org))]
flat.org <- flat.org %>% rename(org_code = country,
                                trust_code = country.1) %>% 
                         mutate(org_code = as.character(org_code),
                                trust_code = as.character(trust_code))
                                
flat.org$record_N <- NA
flat.org$record_N <- as.numeric(flat.org$record_N)
...
# write.csv(flat.org,
#           "Z:/Group_work/PS_AA/PR/PR clinical 2019 Jun-Nov/Data/tidyData/PR_clinical_org_level_data_Jun-Nov_2020-06-16.csv",
#           row.names = FALSE)
...
#           "Z:/Group_work/PS_AA/PR/PR clinical 2019 Jun-Nov/Data/tidyData/benchmarking_quartiles_Jun-Nov_2020-06-16.csv",
#           row.names = FALSE)
...
# write.csv(bmk, file =
# "Z:/Group_work/PS_AA/PR/PR clinical 2019 Jun-Nov/Data/tidyData/benchmarking_2019_Jun-Nov_2020-06-14.csv",
#   row.names = FALSE)
...
dat <- dat %>% filter(gender == "Male" | gender == "Female")
...
# Look at interaction
# Doesn't seem to be an interaction effect...

# dat <- dat %>% filter(gender == "Male" | gender == "Female")
# 
# m1testinteraction <- glmer(MCID_CAT_CRQ_cat ~ 1 + gender + IMD_quintile_all + agecat + CVD_history +
#                       musc_skel_history + SMI_bin_no_missing + anxiety_bin_no_missing + depression_bin_no_missing +
#                       anxiety_bin_no_missing*depression_bin_no_missing +
#                       CAT_score_init_cat + (1 | org_code),
#                     control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)),
#                     data = dat, family = "binomial")
# 
# 
# tidyoutput(m1testinteraction, meth = "Wald")

# This is just comparing it with if you don't take the multi-level structure of PR course into account.
...
dat %>% select(SMI_bin_no_missing, discharge_assess_bin) %>% table()
dat %>% select(anxiety_bin_no_missing, discharge_assess_bin) %>% table()
dat %>% select(depression_bin_no_missing, discharge_assess_bin) %>% table()
dat %>% select(musc_skel_history, discharge_assess_bin) %>% table() %>% prop.table(1)

dat %>% select(CAT_score_init_cat) %>% table()
...
#           "Z:/Group_work/PS_AA/PR/PR clinical 2019 Jun-Nov/Data/tidyData/PR_2019_Jun-Nov_discharge_assess_OR_table.csv",
#           row.names = FALSE)
...
#           "Z:/Group_work/PS_AA/PR/PR clinical 2019 Jun-Nov/Data/tidyData/PR_2019_Jun-Nov_MCID_exercise_OR_table_2020-06-16.csv",
#           row.names = FALSE)


dat %>% select(depression_bin_no_missing, anxiety_bin_no_missing) %>% table()
...
write.csv(MCID_health_status_OR_table,
          "Z:/Group_work/PS_AA/PR/PR clinical 2019 Jun-Nov/Data/tidyData/PR_2019_Jun-Nov_MCID_health_status_OR_table.csv",
          row.names = FALSE)
...
dat %>% select(MCID_CAT_CRQ_cat, SMI_bin_no_missing) %>% table()
dat %>% select(MCID_CAT_CRQ_cat, anxiety_bin_no_missing) %>% table()
dat %>% select(MCID_CAT_CRQ_cat, depression_bin_no_missing) %>% table()
...
# Don't want to include the people who were not referred in this plot, so we don't do the thing below.
# datKMref_to_start_non_AECOPD$ref_to_start_days[is.na(datKMref_to_start_non_AECOPD$ref_to_start_days)] <- 1000
...
ggsave(
"Z:/Group_work/PS_AA/PR/PR clinical 2019 Jun-Nov/Data/tidyData/Survival curve plots/ref_to_start_stable_COPD.png",
...
survfit(Surv(datKM_assess_to_discharge$assess_to_discharge_days, datKM_assess_to_discharge$discharge_assess_bin_num) ~ 
  ...
  plot_survfit(ci = TRUE, legend.title = "Country", xmax = 180) + #, xbreaks = seq(0, 48, 6)) + 
  labs(x = "Time (days)", y = "Percentage of patients who have been discharged from PR following assessment (%)") +
  theme(text=element_text(size=13, family="Calibri"))
        
        
ggsave(
  "Z:/Group_work/PS_AA/PR/PR clinical 2019 Jun-Nov/Data/tidyData/Survival curve plots/assess_to_discharge.png",
  width = 20, height = 20, units = "cm")
...
# hist(dat$test_value_ISWT_diff) # normal
# hist(dat$test_value_6MWT_diff) # normal
# hist(dat$test_value_ESWT_diff) # Not normal
# hist(dat$CAT_score_diff, breaks = 30) # normal
# hist(dat$CRQ_dyspnoea_diff, breaks = 30) # not normal
# hist(dat$CRQ_fatigue_diff, breaks = 30) # basically normal
# hist(dat$CRQ_emotion_diff, breaks = 30) # normal
# hist(dat$CRQ_mastery_diff, breaks = 30) # basically normal

              
              summary(dat$rec_sess_home_no)
              str(dat$rec_sess_home_no)
              summary(dat$prog_type)
              mediSumRound(dat, "rec_sess_home_no", 0)
              
              
              
              table(dat$PR_location, dat$prog_type)
              
              dat %>% select(rec_sess_centre_indiv_no) %>% 
                filter(rec_sess_centre_indiv_no > 0) %>% nrow()
